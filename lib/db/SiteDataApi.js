...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  // 新增文章系列展示功能：保留精简的 posts 列表供前端抽屉/系列组件使用
  if (props.allPages && Array.isArray(props.allPages)) {
    const publishedPosts = props.allPages.filter(p => p && p.type === 'Post' && p.status !== 'Invisible')
    props.posts = cleanPostSummaries(publishedPosts)
    if (process.env.NODE_ENV === 'development') {
      console.log(`[DEBUG-POSTS] ${fullSlug}: allPages=${props.allPages.length}, publishedPosts=${publishedPosts.length}`)
    }
  } else {
    // 文章详情页：allPages 可能已被缓存清理，用 latestPosts 作为 fallback
    // latestPosts 包含已发布的 Post（含 series/number 字段）
    if (props.latestPosts && Array.isArray(props.latestPosts) && props.latestPosts.length > 0) {
      props.posts = cleanPostSummaries(props.latestPosts)
      if (process.env.NODE_ENV === 'development') {
        console.log(`[DEBUG-POSTS] ${fullSlug}: using latestPosts fallback, count=${props.posts.length}`)
      }
    }
  }
...
async function convertNotionToSiteData(
  ...
  // [DEBUG] 临时调试：输出 Notion schema 字段名，确认 series/number 是否存在
  if (process.env.NODE_ENV === 'development') {
    const schemaFieldNames = Object.values(schema).map(s => `${s.name}(${s.type})`).join(', ')
    console.log(`[${traceId}] [DEBUG] Notion Schema 字段: ${schemaFieldNames}`)
  }
  ...
  for (const id of pageIds) {
    ...
    if (properties) {
      // [DEBUG] 临时调试：输出每篇文章的 series/number 字段值
      if (process.env.NODE_ENV === 'development' && properties.type === 'Post') {
        console.log(`[${traceId}] [DEBUG-POST] title="${properties.title}" series=${JSON.stringify(properties.series)} number=${JSON.stringify(properties.number)}`)
      }
