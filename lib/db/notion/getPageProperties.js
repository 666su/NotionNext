...
export default async function getPageProperties(
  ...
  for (let i = 0; i < rawProperties.length; i++) {
    ...
    if (schema[key]?.type && !excludeProperties.includes(schema[key].type)) {
      const textVal = getTextContent(val)
      // 新增文章系列展示功能：text/title 类型确保返回字符串而非数组
      // Notion API 的 title/text 属性 getTextContent 可能返回 ["value"] 数组
      properties[schema[key].name] = Array.isArray(textVal) ? textVal.join('') : textVal
    } else {
...
export function adjustPageProperties(properties, NOTION_CONFIG) {
  ...
  } else if (properties.type === 'Notice') {
    // 新增文章系列展示功能：公告也需要可跳转的链接
    // slug 无效（为空或 '#'）时不生成坏链接，保持 '#' 由前端隐藏入口
    const rawSlug = properties.slug?.trim()
    if (rawSlug && rawSlug !== '#') {
      properties.slug = generateCustomizeSlug(properties, NOTION_CONFIG)
      properties.href = properties.slug ?? properties.id
    } else {
